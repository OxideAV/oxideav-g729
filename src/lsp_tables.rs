...
//! verbatim in the reference implementation (`TAB_LD8K.C`). All values
//! are in `Q13` fixed-point exactly as the spec defines them — the
//! decoder body consumes them unchanged.
//!
//! Layout / sizing (per ITU-T G.729 reference C source `TAB_LD8K.C`):
//! - [`LSPCB1_Q13`]: first-stage codebook, `NC0 = 128` entries of `M = 10`
//!   components each (`L1` field, 7 bits).
//! - [`LSPCB2_Q13`]: second-stage codebook, `NC1 = 32` entries of `M = 10`
//!   components each. The L2 (5 bits) field selects a row whose **low
//!   half** (indices `0..NC=5`) is added to `LSPCB1[L1]`'s low half; the
//!   L3 (5 bits) field selects a row whose **high half** (indices
//!   `NC..M`) is added to `LSPCB1[L1]`'s high half. See §3.2.4 and
...
//!
//! All values transcribed from the ITU-T G.729 reference source code
//! `TAB_LD8K.C` (Software Package Release 2, November 2006). The tables
//! are normative spec data per Recommendation §3.2.4 and Annex C.
...
/// component of the i-th codeword. Verbatim from `TAB_LD8K.C` `lspcb1`.
...
/// Second-stage LSP codebook (Q13). `LSPCB2_Q13[i][j]` is the j-th
/// component of the i-th 10-component codeword. The L2 (5-bit) index
/// selects a row whose **low half** (j = 0..5) is summed with
/// `LSPCB1[L1]`'s low half; the L3 (5-bit) index selects a (possibly
/// different) row whose **high half** (j = 5..10) is summed with
/// `LSPCB1[L1]`'s high half.
///
/// Verbatim from `TAB_LD8K.C` `lspcb2[NC1][M]`.
pub const LSPCB2_Q13: [[i16; M]; NC1] = [
...
///
/// Verbatim from `TAB_LD8K.C` `fg[2][MA_NP][M]`.
...
/// formula in §3.2.4 Eq. (19). Verbatim from `TAB_LD8K.C` `fg_sum[2][M]`.
...
/// Verbatim from `TAB_LD8K.C` `fg_sum_inv[2][M]`.
...
/// Initial value of the MA-predictor history (Q13), one entry per LSF
/// component. Equals `pi * (j+1) / (M+1)` mapped to Q13 using
/// `pi << 13 = 25736` (truncated). See `freq_prev_reset[]` in
/// `LSPDEC.C`.
pub const FREQ_PREV_RESET_Q13: [i16; M] = [
...
mod tests {
    ...
    #[test]
    fn lspcb1_spec_witness_rows() {
        // Cross-reference selected rows against the spec C source listing
        // (TAB_LD8K.C) — guards against transcription drift if the table
        // is ever re-edited.
        assert_eq!(
            LSPCB1_Q13[0],
            [1486, 2168, 3751, 9074, 12134, 13944, 17983, 19173, 21190, 21820]
    ...
    #[test]
    fn freq_prev_reset_matches_uniform_spread() {
        // freq_prev_reset[j] = floor(pi * (j+1) / (M+1) * 2^13)
        // with pi*2^13 = 25736 (truncated).
        let pi_q13: i32 = 25736;
        for j in 0..M {
            let expected = (pi_q13 * (j as i32 + 1) / (M as i32 + 1)) as i16;
            assert_eq!(FREQ_PREV_RESET_Q13[j], expected);
        }
    }
}
